...
class EngineBuilder:
    ...
    def __build_engine(self,
        ...
        with self.builder.build_engine(self.network, config) as engine:
            self.wb = engine
            self.save_path.write_bytes(engine.serialize())
        self.logger.log(
            trt.Logger.WARNING, f'Build tensorrt engine finish.\n'
            f'Save in {str(self.save_path.absolute())}')
    ...
    def build(self,
        ...
        self.__build_engine(precision, save_path, input_shape, iou_thres, conf_thres, topk,
                            with_profiling)
